...
class BlindTestView(discord.ui.View):
    ...
    async def stop(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.edit_message(
            embed=discord.Embed(
                title=":white_check_mark: Le bot a été déconnecté du salon vocal"
            ),
    ...
    async def launch_music(self, interaction: discord.Interaction):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                if (
                    self.settings["answers"]["title"]
                    and "title" not in founded
                    and music["title"].lower() in answers
                ):
                    founded["title"] = key
                    self.score[key] = self.score.get(key, 0) + 1
                    new_found = True
                if (
                    self.settings["answers"]["artist"]
                    and "artist" not in founded
                    and music["artist"].lower() in answers
                ):
                    founded["artist"] = key
                    self.score[key] = self.score.get(key, 0) + 1
                    new_found = True
                if (
                    self.settings["answers"]["game"]
                    and "game" not in founded
                    and music["game"].lower() in answers
                ):
                    founded["game"] = key
                    self.score[key] = self.score.get(key, 0) + 1
                    new_found = True
        ...
        self.score = dict(
            sorted(self.score.items(), key=lambda item: item[1], reverse=True)
        )
        await interaction.channel.send(
            embed=embed_score(self.score, good_answers, self.is_all_find(founded)),
            view=self,
        )
